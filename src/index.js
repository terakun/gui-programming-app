    ...
    return this.state.graph.edges.map((nodes, node_from) => {
      return (
        nodes.map((node_to, index) => {
          ...
          return (
            <Line keys={this.nodes_num * node_from + index} x1={x1} y1={y1} id1={node_from} x2={x2} y2={y2} id2={node_to} thickness={1} addLine={this.addLine.bind(this)} color="black" />
          );
        }));
    });
  }
  ...
  render() {
    ...
    return (
      <div>
            ...
            <div>
              {(() => {
                if (this.state.isMouseDown) {
                  return (<Line ref='line' x1={selectedcompX} y1={selectedcompY} id1={-1} x2={mouseX} y2={mouseY} id2={-1} thickness={1} color="black" addLine={this.addLine.bind(this)} />);
                }
              })()}
            </div>
          </div>
        ...
        <button onClick={() => { this.connection = new WebSocket("ws:127.0.0.1:8000"); }}>再接続</button>
